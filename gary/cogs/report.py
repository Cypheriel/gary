from discord import ApplicationContext, message_command, user_command, Member, User, Message, ui, Interaction, \
    SelectOption, InteractionResponse, ButtonStyle, Embed, EmbedField, CategoryChannel, ForumChannel, \
    StageChannel
...
class ReportView(ui.View):
    def __init__(self, reported_message: Message | None = None, user: User | Member | None = None):
        super().__init__()
    ...
    @ui.string_select(placeholder="Select rule violation(s)", min_values=1, max_values=9, options=RULES_OPTIONS)
    ...
    async def report(self, _button: ui.Button, interaction: Interaction):
        ...
        if not report_channel or isinstance(report_channel, StageChannel | ForumChannel | CategoryChannel):
            return

        await report_channel.send(embeds=[
            Embed(
                title=f"Report",
                fields=[
                    EmbedField(
                    ...
                    EmbedField(
                        ...
                        value=f"\"{self.reported_message.content}\" ([link]({self.reported_message.jump_url}))"
                              if self.reported_message else "N/A"
                    ),
                ],
            ),
            Embed(
...
class Report(Cog):
    ...
    async def report_message(self, ctx: ApplicationContext, message: Message):
        await ctx.send_response(view=ReportView(reported_message=message), ephemeral=True)
